...
const emptyForm: FormData = { name: '', color: '#1976d2', icon: '', value: '' }

// ─── Component ───────────────────────────────────────────

export default function StatusConfigCrud({ type, title, baseUrl: baseUrlProp }: Props) {
  ...
  const baseUrl = baseUrlProp ?? `/admin/customerscontracts/config/${type}`
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingItem) {
        const res = await apiClient.put<{ success: boolean; data: StatusItem }>(
        ...
        const res = await apiClient.post<{ success: boolean; data: StatusItem }>(baseUrl, payload)
  ...
  const handleOpenDelete = (item: StatusItem) => {
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={{ fontWeight: 'bold' }}>{t.statusCrudColor}</TableCell>
                  <TableCell sx={{ fontWeight: 'bold' }}>{t.statusCrudValue}</TableCell>
