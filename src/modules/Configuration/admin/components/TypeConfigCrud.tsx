...
export default function TypeConfigCrud({ baseUrl: baseUrlProp, title: titleProp }: Props) {
  ...
  const baseUrl = baseUrlProp ?? '/admin/customersmeetings/config/types'
  const title = titleProp ?? t.meetingTypeTitle
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingItem) {
        const res = await apiClient.put<{ success: boolean; data: TypeItem }>(
        ...
        const res = await apiClient.post<{ success: boolean; data: TypeItem }>(baseUrl, payload)
